...
class EnsembleAnalyzer:
    ...
    def __init__(self):
        ...
        # Expected Value tracking (conservative, quality-focused)
        self.ev_tracker = {
            'total_trades': 0,
            'wins': 0,
            'losses': 0,
            'avg_win': 22.0,  # Conservative: 22 pips average win
            'avg_loss': 14.0,  # Conservative stop: 14 pips average loss
            'win_rate': 0.0
        }
        
        # Dynamic confidence thresholds based on ADX (conservative, quality-focused)
        self.confidence_thresholds = {
            'strong_trend': {'adx_min': 40, 'threshold': 0.75},  # Slightly down from 0.80
            'moderate_trend': {'adx_min': 25, 'threshold': 0.59},  # Minimal change from 0.60
            'weak_trend': {'adx_min': 0, 'threshold': 0.50}  # Minimum 50% confidence required
        }
    ...
    def _calculate_expected_value(self, probability, market_regime):
        ...
        try:
            # Get historical performance data with conservative defaults
            avg_win = self.ev_tracker.get('avg_win', 22)  # Default 22 pips
            avg_loss = self.ev_tracker.get('avg_loss', 14)  # Default 14 pips
            
            # Adjust for market regime
            if market_regime.get('regime') == 'TRENDING':
                avg_win *= 1.3  # Higher wins in trending markets
                avg_loss *= 0.9  # Slightly better stop placement
            elif market_regime.get('regime') == 'RANGING':
                avg_win *= 0.9  # Lower wins in ranging markets
                avg_loss *= 1.0  # Keep loss consistent
    ...
    def _meta_labeling_decision(self, data, probability, market_regime):
        """Meta-labeling: decide whether to act on the signal (conservative threshold)"""
        try:
            if self.meta_classifier is None:
                # If no meta-classifier trained, use conservative heuristics
                if probability > 0.59 and market_regime.get('adx', 0) > 19:
